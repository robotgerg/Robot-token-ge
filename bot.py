...
D = uptime.strftime("%d")
A = uptime.strftime("%Y")
bot = telepot.Bot('302888891:AAHVBozfKAvipC5Mn6hQ_a02HXUXAXHbJUo')
NomeBot = "GE"
...
### BOT PARAMTS
    ...
    print(msg)
    feed = feedparser.parse('http://guiadoestudante.abril.com.br/feed')
    encoded = base64.b64encode(feed.entries[0].published)
    txt = msg['text'].split(' ')
...
###DICAs
 ...
 #NovosComandos
      ...
      markup = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='NOTÍCIAS')]+[KeyboardButton(text='MENU DO SITE')],[KeyboardButton(text='CATEGORIAS')],[KeyboardButton(text='COMPRE')]+[KeyboardButton(text='PRODUTOS')],[KeyboardButton(text='SOBRE O GUIA')],[KeyboardButton(text='VOLTAR')]], one_time_keyboard=True)
...
###Comandos
    elif data['/new'] !=encoded:
        POSTAGEN="↪ *GUIA DO ESTUDANTE - Notícias Recentes*"+"\n\n"+"▶"+feed.entries[0].title+"\n\n"+"▪ "+feed.entries[0].link+"\n\r"
        post="*▶  PUBLICAÇÃO*"+"\n\n"+feed.entries[0].summary+"\n\n"+feed.entries[0].published+"\n\r"
        bot.sendMessage(chat_id=chat_id, text=POSTAGEN, parse_mode='Markdown')
        time.sleep(2)
        bot.sendMessage(chat_id=chat_id, text=post, parse_mode='Markdown')
    else:
